...
const blank: TaskView = {id: '', title: '', description: '', status: 'todo', priority: 'none', dueDate: null}
...
function TaskForm({ task, busy, save }: { task: TaskView; busy: boolean; save: (input: SaveInput) => Promise<boolean> }) {
  return <form className="task-form" onSubmit={async event => {
    ...
    const saved = await save({id: task.id || undefined, title: String(values.get('title')), description: String(values.get('description')), status: values.get('status') as TaskInput['status'], priority: values.get('priority') as TaskInput['priority'], dueDate: String(values.get('dueDate')) || null})
    if (saved) {
  ...
  return <section aria-label="Your tasks">
    ...
    {tasks.length === 0 ? <div className="empty"><span aria-hidden="true">✳</span><h2>{search.q || search.status !== 'all' ? 'No tasks match just yet.' : 'Make room for your next idea.'}</h2><p>Add a task or try another filter. Small steps count.</p></div> :
      <ul className="task-list">{tasks.map(task => <li key={task.id} className={`task ${task.status}`}>
        <details><summary><span className="status-dot" aria-hidden="true" /><span className="task-name">{task.title}{task.priority !== 'none' && <span className={`task-meta priority-${task.priority}`}>{priorityLabels[task.priority]} priority</span>}{task.dueDate && <time className="task-meta" dateTime={task.dueDate}>Due {task.dueDate}</time>}</span><span className="badge">{statusLabels[task.status]}</span><span className="edit-hint">Edit ↗</span></summary>

...
test('page persists tasks and navigates URL filters through the real router', async () => {
  ...
  await user.type(screen.getByLabelText('Tags'),' Work, Assessment ')
  await user.click(screen.getByRole('button',{name:'Add task'}))
  await screen.findByText('Route integration task')
  expect(screen.getByText('High priority')).toBeTruthy()
  expect(screen.getByText('Due 2026-09-18')).toBeTruthy()
  expect(screen.getByText('#work')).toBeTruthy()
  await user.click(screen.getByText('Route integration task'))
  await user.clear(screen.getAllByLabelText('Task title')[1])
  await user.type(screen.getAllByLabelText('Task title')[1],'Route integration task edited')
  await user.click(screen.getByRole('button',{name:'Save changes'}))
  await screen.findByText('Route integration task edited')
  await waitFor(() => expect(document.activeElement?.tagName).toBe('SUMMARY'))
  expect(screen.getByText('Route integration task edited').closest('details')!.open).toBe(false)
  await user.type(screen.getByLabelText('Filter by tag'),'WORK')
  await user.type(screen.getByRole('searchbox'),'assess')
  await user.click(screen.getByRole('button',{name:'Search'}))
  await waitFor(() => expect(router.state.location.search).toMatchObject({q:'assess',tag:'work',status:'all'}))
  await waitFor(() => expect(document.querySelector('.loading')).toBeNull())
  expect(screen.getByText('Route integration task edited')).toBeTruthy()
  ...
  await waitFor(() => expect(document.querySelector('.loading')).toBeNull())
  await user.click(await screen.findByText('Route integration task edited'))
  vi.spyOn(window, 'confirm').mockReturnValue(true)
  await user.click(await screen.findByRole('button',{name:'Delete Route integration task edited'}))
  await screen.findByText('No tasks match just yet.')
  vi.restoreAllMocks()
})

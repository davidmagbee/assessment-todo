...
test('an empty list offers a labeled task form and saves a new task', async () => {
  ...
  expect(save).toHaveBeenCalledWith({id: undefined, title:'Ship something good', description:'',status:'todo',priority:'none',dueDate:null})
...
test('existing tasks can be edited, filtered and deleted only after confirmation', async () => {
  ...
  render(<TaskBoard tasks={[{id:'task-a',title:'Review the design',description:'Check contrast',status:'in_progress',priority:'none',dueDate:null}]} search={{q:'',status:'all'}} onSearch={search} onSave={save} onRemove={remove} />)
  ...
  expect(save).toHaveBeenCalledWith({id:'task-a',title:'Review accessibility',description:'Check contrast',status:'in_progress',priority:'none',dueDate:null})
...
test('failed saves preserve input and failed deletion remains retryable', async () => {
  ...
  render(<TaskBoard tasks={[{id:'x',title:'Keep me',description:'',status:'done',priority:'none',dueDate:null}]} search={{q:'',status:'all'}} onSearch={vi.fn()} onSave={vi.fn().mockRejectedValue(new Error('offline'))} onRemove={vi.fn().mockRejectedValue(new Error('offline'))} />)
...
test('priority can be set on creation and is visible on a collapsed task', async () => {
  ...
  render(<TaskBoard tasks={[{id:'priority',title:'Important',description:'',status:'todo',priority:'high',dueDate:null}]} search={{q:'',status:'all'}} onSearch={vi.fn()} onSave={save} onRemove={vi.fn()}/>)
...
test('due dates are displayed as calendar dates and can be removed from an edit', async () => {
  const user = userEvent.setup(), save = vi.fn().mockResolvedValue(undefined)
  render(<TaskBoard tasks={[{id:'deadline',title:'Submit',description:'',status:'todo',priority:'none',dueDate:'2026-09-18'}]} search={{q:'',status:'all'}} onSearch={vi.fn()} onSave={save} onRemove={vi.fn()}/>)
  ...
  await user.click(screen.getByRole('button',{name:'Save changes'}))
  expect(save).toHaveBeenLastCalledWith(expect.objectContaining({id:'deadline',dueDate:null}))

...
## Keyboard lifecycle follow-up — 2026-09-18

After the feature push, live keyboard verification motivated moving post-save focus restoration into a layout effect tied to the revealed task list. Saves focus the closed task summary; tasks removed by the active filter return focus to search. Command listeners now detach during Suspense hiding, avoiding null refs when typing during refresh. Added filtered-out focus and suspended-command regressions; router integration now edits and checks focus after loading updated data. Native browser Shift+Tab from the saved task reached Add task, verifying the expected keyboard position. Full-page layout inspected; local create/edit/reload, metadata, combined tags/text/status and inside/outside shortcuts exercised. Removed the exact assistant-created QA row. Validation: 72 tests; statements 241/241, branches 107/107, functions 81/81, lines 192/192; typecheck/build pass.

## Enhancement release verification — 2026-09-18

Six atomic code commits pushed through d46ffa2. Vercel production deployment assessment-todo-c52yqk8kv-davidmagbees-projects.vercel.app reports Ready; canonical assessment-todo.vercel.app serves the new controls. Hosted compiled RPC verified metadata create/update/clear, normalized tag substring/exact/status filters, private/no-store SSR and cross-owner update/delete rejection. Hosted browser verified Alt/Option+F from the palette focuses search. Exact QA fixtures removed; local outside-shortcut preference restored off. GitHub visibility rechecked PUBLIC. README and public retrospective planning summary updated. No hosted inbox/account acceptance claim added.
